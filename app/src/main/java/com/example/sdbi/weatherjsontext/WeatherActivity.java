...
public class WeatherActivity extends AppCompatActivity {
	...
	*/
	...
	ImageView[] imgWeatherTypes = new ImageView[]{imgWeatherType1, imgWeatherType2, imgWeatherType3, imgWeatherType4, imgWeatherType5, imgWeatherType6};
	/*
	 * 折线图
	 */
	...
	*/
	...
	private void showWeatherInfo(Weather weather) {
		...
		tvData.setText(weather.today.date_y);
		tvWeek.setText(weather.today.week);
		tvWendu.setText(weather.sk.temp + "度");
		tvWeatherType.setText(weather.today.weather);
		setImg(weather.today.weather);
		ivWeatherType.setImageResource(imgId);
		TextView[] tvDays = new TextView[]{tvDay1, tvDay2, tvDay3, tvDay4, tvDay5, tvDay6};
		TextView[] tvWeeks = new TextView[]{tvWeek1, tvWeek2, tvWeek3, tvWeek4, tvWeek5, tvWeek6};
		TextView[] tvWeatherTypes = new TextView[]{tvWeatherType1, tvWeatherType2, tvWeatherType3, tvWeatherType4, tvWeatherType5, tvWeatherType6};
		ImageView[] imgWeatherTypes = new ImageView[]{imgWeatherType1, imgWeatherType2, imgWeatherType3, imgWeatherType4, imgWeatherType5, imgWeatherType6};
		for (int i = 0; i < 6; i++) {
			Future future = weather.futureList.get(i);
			if (future.date.substring(4, 5).equals("0")) {
				tvDays[i].setText(future.date.substring(5, 6) + "月" + future.date.substring(7, 8) + "号");
			} else {
				tvDays[i].setText(future.date.substring(4, 6) + "月" + future.date.substring(7, 8) + "号");
			}
			tvWeeks[i].setText(future.week);
			tvWeatherTypes[i].setText(future.weather);
			setImg(future.weather);
			imgWeatherTypes[i].setImageResource(imgId);
		}
		/*
	...
	private void setImg(String str) {
		switch (str) {
			case "晴":
				imgId = R.drawable.vector_drawable_icon_qing;
				break;
			case "多云":
				imgId = R.drawable.vector_drawable_icon_duoyun;
				break;
			case "阴":
				imgId = R.drawable.vector_drawable_icon_yin;
				break;
			case "阵雨":
				imgId = R.drawable.vector_drawable_icon_xiaoyu;
				break;
